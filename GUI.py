...
def make_window():
    ...
    leftCol = [
        [sg.Listbox(values=get_file_list('Antennas'), select_mode=sg.SELECT_MODE_EXTENDED, size=(45,10), bind_return_key=True, key='-DEMO LIST-')],
        [sg.Text('Antenna Filter:', tooltip=filterTip), sg.Input(size=(33, 1), focus=True, enable_events=True, key='-FILTERA-', tooltip=filterTip)],
        [sg.Button('Select')],
        [sg.Listbox(values=get_file_list('Satellites'), select_mode=sg.SELECT_MODE_EXTENDED, size=(45,10), bind_return_key=True, key='-DEMO LIST-')],
        [sg.Text('Satellite Filter:', tooltip=filterTip), sg.Input(size=(33, 1), focus=True, enable_events=True, key='-FILTERS-', tooltip=filterTip)],
        [sg.Button('Select')],
    ...
    midCol = [
        ...
        [sg.Input(size=(33, 1), focus=True, enable_events=True, key='-FILTERTH-', tooltip=threshTip)],
        [sg.Button('Continue')],
    ...
    rightCol = [
        ...
        [sg.Input(size=(33, 1), focus=True, enable_events=True, key='-FILTERAZ-', tooltip=aziTip)],
        [sg.Text('Choose Elevation:')],
        [sg.Input(size=(33, 1), focus=True, enable_events=True, key='-FILTEREL-', tooltip=eleTip)],
        [sg.Button('Continue')],
        [sg.Canvas(key='-CANVAS3-')], 
        [sg.Button('Cancel')]
    ] 

    layout = [[sg.Text('antenna control e', font='Any 20')],
              [sg.Column(leftCol, element_justification='c'), sg.Column(midCol, element_justification='c'), sg.Column(rightCol, element_justification='c')],
    ]
