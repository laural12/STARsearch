...
class RotationBase:
    ...
    # LIMIT_ENC_EL =
    # ANTENNA_INPUT = # ????
    azSensor = AzimuthHallSensors(azInitial = 0, pinNum = AZ_ENC1)
    elSensor = ElevationHallSensors(EInitial= 0, pinNum = EL_ENC1)
    ...
    def __init__(self, GUI_test=False):
        ...
        if not self.GUI_test:
            ...
            #wpi.pullUpDnControl(int pin, int pud)
    ...
    def elTurnRight(self):
        ...
        self.azSensor.movingClockwise(True)
        # wpi.digitalWrite(EL_ENABLE, HIGH)  # el enable
    ...
    def getEl(self):
        return self.elSensor.get_elevation_angle()
    def getAz(self):
        return self.azSensor.get_azimuth()
